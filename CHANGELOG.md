...
### Fixed
- Mobile: every card in the CV page grid shrank to its own content width instead of filling the column — most visible on the Download card, which rendered at roughly half the screen width next to full-width cards below it (181px of a 375px container). The base `.page-grid` rule sets `align-items: flex-start`, which in a grid means "top-align the columns"; the `max-width: 959px` block switches the same element to `display: flex; flex-direction: column`, where that property flips to the cross axis and means "shrink to content" instead. Restored with an explicit `align-items: stretch` in the mobile block
...
### Changed
- `docs/INFO.md` (Russian-only) split into `docs/INFO_en.md` (full English translation, now the canonical reference linked from README, ARCHITECTURE.md, `astro.config.mjs`, `content.config.ts`, `HomePage.astro` and `init.mjs`) and `docs/INFO_ru.md` (the original Russian content), cross-linked at the top of each. README's `site.yml` and "How to edit your data" sections trimmed of detail that duplicated `docs/INFO_en.md`, replaced with a link
